-- FINAL TEST ACCESS FOR ONLY swathi.24cs@kct.ac.in
-- Run this after the earlier admission workflow migrations.
-- It grants full APPLICATION workflow access: view, edit, save and approve.
-- It does NOT grant Supabase Dashboard or database-administrator access.
...
begin
  if not public.is_authorized_review_staff() then
    raise exception 'Only an authenticated KIA staff account or the authorized test account can access applications.' using errcode = '42501';
  end if;
  return query
...
begin
  if not public.is_authorized_review_staff() then
    raise exception 'Only an authenticated KIA staff account or the authorized test account can edit applications.' using errcode = '42501';
  end if;
